...
// askllmr(R code tutor)自訂 UI 事件:把 question 的單行 TextBox 換成多行 textarea。
// 機制與 askllm.js 相同(見該檔案註解),此為複本,識別字串改對應此分析。

const AREA_ID = 'askllmr-question-area';
const LABEL_TEXT = 'What should the R code do?';

// 各 provider 的預設模型(與 R/llm-providers.R、jamovi/js/askllm.js 對照表一致)
...
// A) 以 LABEL_TEXT 為 DOM 錨點,取其後第一個 input(零副作用,最穩)
...
function findQuestionInput(ui, root) {
    ...
    if (orig !== null && getOption(ui, 'submit') === false) {
        const SENTINEL = '__askllmr_locator__';
        try {
...
function buildTextarea(ui, root, initial) {
    ...
    ta.rows = 5;
    ta.value = initial || '';
    ta.placeholder = 'Describe what the R code should do / 描述你想要的 R 程式碼(可多行)';
    ta.style.cssText = [
        'width: ' + width + 'px',
        'max-width: none',
...
function init(ui) {
    ...
    if ((initial === null || initial === '') && inp)
        initial = inp.value;
    if (initial === '__askllmr_locator__')
        initial = '';
    ...
    updated(ui) {
        ...
        try {
            ...
            if (document.activeElement !== ta) {
                ...
                if (val !== null && val !== '__askllmr_locator__')
                    ta.value = val;
            }
        } catch (e) { }
    }
