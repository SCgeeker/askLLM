...
test_that('guide_text: 英文區塊在中文區塊之前,語言不交錯', {
    txt <- .askllm_guide_text()
    expect_true(regexpr('How to use Ask LLM', txt) < regexpr('如何使用 Ask LLM', txt))
...
test_that('waiting_text: 英文區塊在中文區塊之前,語言不交錯', {
    txt <- .askllm_waiting_text('NVIDIA NIM', 'meta/llama-3.1-8b-instruct')
    expect_true(regexpr('Waiting for a response', txt) < regexpr('正在等候', txt))
})
...
test_that('caveat_text: 英文區塊在中文區塊之前,語言不交錯', {
    on <- .askllm_caveat_text(has_catalog = TRUE)
    expect_true(regexpr('This answer is generated by an LLM', on) <
                regexpr('回覆由 LLM 生成', on))

    off <- .askllm_caveat_text(has_catalog = FALSE)
    expect_true(regexpr('This answer is generated by an LLM', off) <
                regexpr('回覆由 LLM 生成', off))
})

# ---- 構件 4:.askllm_caveat_text(r_code=, has_rj_env=, has_material=) -------
#
# M-A0:.askllm_caveat_text() 已退回單參數 has_catalog 原簽名,以下 4 案例暫
# skip,待 M-A3 改指向新分析 askllmr 的 .askllmr_caveat_text()。

test_that('caveat_text: 舊兩參數呼叫(has_catalog 具名)與升版前逐字相同', {
    skip('M-A3: 改指向 askllmr')
    expect_identical(.askllm_caveat_text(has_catalog = TRUE),
                      .askllm_caveat_text(has_catalog = TRUE, has_rj_env = NA,
                                           has_material = NA, r_code = FALSE))
    expect_identical(.askllm_caveat_text(has_catalog = FALSE),
                      .askllm_caveat_text(has_catalog = FALSE, has_rj_env = NA,
                                           has_material = NA, r_code = FALSE))
    # 無參數呼叫、r_code 預設值皆不應新增任何內容
    expect_identical(.askllm_caveat_text(), .askllm_caveat_text(r_code = FALSE))
})

test_that('caveat_text: r_code=TRUE 時加入「R code 未執行」提醒(中英皆有)', {
    skip('M-A3: 改指向 askllmr')
    txt <- .askllm_caveat_text(has_catalog = TRUE, r_code = TRUE)
    expect_true(grepl('NOT executed', txt, fixed = TRUE))
    expect_true(grepl('Rj', txt, fixed = TRUE))
    expect_true(grepl('並未由 askLLM 執行', txt, fixed = TRUE))
})

test_that('caveat_text: r_code=TRUE + has_rj_env=TRUE 時加入「已接地」句', {
    skip('M-A3: 改指向 askllmr')
    on  <- .askllm_caveat_text(r_code = TRUE, has_rj_env = TRUE)
    off <- .askllm_caveat_text(r_code = TRUE, has_rj_env = FALSE)
    na  <- .askllm_caveat_text(r_code = TRUE, has_rj_env = NA)

    expect_true(grepl('grounded', on, ignore.case = TRUE))
    expect_true(grepl('接地', on, fixed = TRUE))

    expect_true(grepl('No Rj environment', off, fixed = TRUE))
    expect_true(grepl('未附上 Rj 環境', off, fixed = TRUE))

    # has_rj_env=NA 時不出現 rj 兩態句(接地/未附上),但仍保留「未執行」提醒
    expect_false(grepl('grounded', na, ignore.case = TRUE))
    expect_false(grepl('接地', na, fixed = TRUE))
    expect_false(grepl('No Rj environment', na, fixed = TRUE))
    expect_false(grepl('未附上 Rj 環境', na, fixed = TRUE))
    expect_true(grepl('NOT executed', na, fixed = TRUE))
})

test_that('caveat_text: r_code=FALSE 時,has_rj_env 不論何值都不影響輸出', {
    skip('M-A3: 改指向 askllmr')
    base <- .askllm_caveat_text(has_catalog = TRUE, r_code = FALSE)
    with_rj_true  <- .askllm_caveat_text(has_catalog = TRUE, has_rj_env = TRUE,  r_code = FALSE)
    with_rj_false <- .askllm_caveat_text(has_catalog = TRUE, has_rj_env = FALSE, r_code = FALSE)
    expect_identical(base, with_rj_true)
    expect_identical(base, with_rj_false)
})
