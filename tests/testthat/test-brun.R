...
# ---- 構件 4 → M-A3:.askllmr_caveat_text(has_rj_env=) 改指向新分析 ---------
#
# M-A0 已把 .askllm_caveat_text() 退回單參數 has_catalog 原簽名(諮詢分析
# askllm 不再有 R-code 意圖)。以下案例 M-A3 改指向 R code tutor(askllmr)的
# .askllmr_caveat_text()(定義於 R/r-tutor.R):固定句 + .askllm_rj_caveat_lines()
# 三態句 + 末句(數值以 jamovi 為準),先英文整段再中文整段。

test_that('askllmr_caveat_text: 英文區塊在中文區塊之前,語言不交錯', {
    txt <- .askllmr_caveat_text()
    expect_true(regexpr('generated by an LLM', txt) < regexpr('LLM 生成', txt))
    expect_true(regexpr('jamovi is correct', txt) <
                regexpr('以 jamovi 為準', txt))
})

test_that('askllmr_caveat_text: 預設(has_rj_env 未傳)含「R code 未執行」提醒(中英皆有)', {
    txt <- .askllmr_caveat_text()
...
test_that('askllmr_caveat_text: has_rj_env=TRUE/FALSE 兩態句與 NA 互斥(三態)', {
    on  <- .askllmr_caveat_text(has_rj_env = TRUE)
    off <- .askllmr_caveat_text(has_rj_env = FALSE)
    na  <- .askllmr_caveat_text(has_rj_env = NA)
...
test_that('askllmr_caveat_text: 末句提醒數值以 jamovi 為準(中英皆有)', {
    txt <- .askllmr_caveat_text(has_rj_env = TRUE)
