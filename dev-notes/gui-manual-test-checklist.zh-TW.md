...
- [ ] Provider 下拉出現 **OpenRouter**，位置在 **Gemini 之後、GitHub 之前**。
- [ ] 選 OpenRouter → Model 欄自動帶入 **`openai/gpt-oss-20b:free`**（若沒自動帶入是 js `PROVIDER_DEFAULTS` 未生效，記錄）。
- [ ] 已設 `OPENROUTER_API_KEY`（`setx` 或 `.Renviron`，設完**完全重啟 jamovi**）→ 填問題 → Submit → 得到真實回覆。
- [ ] 未設金鑰時 → 顯示取得金鑰指引（含 `OPENROUTER_API_KEY` 與 openrouter.ai/keys）。

### 項目 3 — Test Connection（非課金疎通檢查）

- [ ] Submit 附近出現 **Test Connection** CheckBox。
- [ ] 勾 Test Connection（不勾 Submit）→ 對各 provider 顯示對應文案（**均附「金鑰來源」**）：
  - [ ] NVIDIA / OpenRouter：`✓ 端點可連線（此供應商的 /models 不驗證金鑰…）`
  - [ ] Gemini：`✓ 金鑰有效`（金鑰對）／`✗ 金鑰無效（400）`（故意打錯金鑰）
  - [ ] GitHub Models：`✓ 金鑰有效（404）`（金鑰對）／`✗ 未授權（401）`（打錯）
  - [ ] Ollama：起服務→`✓ 本機服務可連線`；未起→`✗ 服務未啟動`
- [ ] **Test 優先**：同時勾 Test Connection 與 Submit → 只做連線檢查、**不呼叫 LLM**（供應商後台應顯示零 completion 消耗）。
- [ ] 故意打錯金鑰做一次 Gemini/GitHub → 確認 `✗` 文案與 HTTP 碼正確。

---

## 4. 整合／回歸行為

- [ ] **等待狀態**：送出後、回覆前，畫面顯示「正在等候…／Waiting…」而非卡死。
- [ ] **快取回放**：同樣參數再按一次 → meta 出現 `· cached`、不重打 API。
- [ ] **快取失效**：只改 Persona 或 Prompt language 或 Custom prompt → **重新呼叫**（非 cached），證明 role/lang/systemPrompt 已進防抖 payload。
- [ ] **查證提醒**：回覆下方仍有 ⚠ caveat（選單路徑是否比對本機清單，依 includeCatalog）。
- [ ] 開一份**舊 .omv**（1.1 版存的）→ 結果能正常還原、不報錯（確認新增選項未破壞相容）。
