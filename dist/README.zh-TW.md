...
範例:`askLLM_1.3.2_win64.jmo` 代表:

- `askLLM` —— 模組名稱
- `1.3.2` —— 模組版本(來自 `DESCRIPTION` / `jamovi/0000.yaml`)
...
jamovi 版本這一項則沒那麼硬。不同 jamovi 發行版所綁的 R 與 CRAN snapshot 確實不同,所以無法保證一份建置到處都能跑。能報告的是實測結果:1.3.2 的建置在 jamovi 28.2.0.0–28.3.0.0 上正常運作。若你的平台不同,請依下方步驟從原始碼重新建置,而非直接使用本目錄現有檔案。
